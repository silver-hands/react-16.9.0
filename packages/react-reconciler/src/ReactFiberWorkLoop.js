...
function renderRoot(
  ...
  // 由于 root 是共享的，所以有可能在一次之前的更新过程中，将本次的更新任务完成了，那么这次更新什么都不做
...
// 这种情况只会发生在调用了 batch.commit() API 时候的情况
  ...
  if (workInProgress !== null) {
    ...
    // executionContext 更新，加上 RenderContext 的标记
    executionContext |= RenderContext;
    // ReactCurrentDispatcher 赋默认值
    ...
    // isSync 表示已经过期了
    // expirationTime !== Sync 表示这不是一个同步任务
    // 其实就是一个异步任务过期了，会将过期时间重置，重新加入调度
    // 由于新生成的任务 task 优先级和之前的一样，会排在任务的第一个
    // 再次进入 renderRoot 后，大概率不满足 currentTime < expirationTime 的条件，大概率是一样的
    // 是因为 requestCurrentTime 会抹平 10 ms 内的误差
    // 跟新 fiber 树的时候，优先级不够的会不做更新
    // 将 renderRoot 绑定的过期时间变成当前时间，变相提升了 fiber 树上节点的优先级
    ...
    if (isSync) {
      ...
      if (expirationTime !== Sync) {
        ...
        if (currentTime < expirationTime) {
          ...
          // 注意这个 return 的函数会在 flushTask 函数中作为新任务的 task
      ...
      // 任务没过期，一定会执行，那就把 currentEventTime 重置
  ...
  // 至此，保证了 fiber 树的每个节点的状态都是一致的。
  // 接下来会执行 commit 步骤/或者是又有新的任务被挂起了，等待挂起结束再去 commit
  stopFinishedWorkLoopTimer();
  // root.current.alternate 其实就是上面完成变更的 fiber
  ...
  // 也就是当root被处理完，根据它的状态，来判断后续是进行commit、阻止commit并重新render、等待一段时间再去commit、还是报错。
...
function performUnitOfWork(unitOfWork: Fiber): Fiber | null {
  ...
  //获取当前节点
   // performUnitOfWork 的入参是 workInProgress，这个Fiebr 对象通过 alter 指向原对象
